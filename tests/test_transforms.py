...
def test_seq_trans_null():
    sequence_transform([ast.Num(20)], ast.Num(20))


def test_seq_trans_no_args(mock_qt):
    s = sequence_transform([], ast.Num(20))
    base_seq = ObjectStream(ast.Name(id='dude'))
    new_seq = s.sequence(base_seq, {}, mock_qt)
...
def test_seq_trans_one_args_no_repl(mock_qt):
    a = ast.Num(10)
    s = sequence_transform([a], ast.Num(20))
    base_seq = ObjectStream(ast.Name(id='dude'))
    new_seq = s.sequence(base_seq, {a: ast.Num(30)}, mock_qt)
    assert ast.dump(new_seq._ast, annotate_fields=False) == "Call(Name('Select', Load()), [Name('dude'), Lambda(arguments([arg('e1000')]), Num(20))], [])"


def test_seq_trans_one_args(mock_qt):
    a = ast.Num(10)
    s = sequence_transform([a], a)
    base_seq = ObjectStream(ast.Name(id='dude'))
    new_seq = s.sequence(base_seq, {a: ast.Num(30)}, mock_qt)
...
def test_seq_trans_attr(mock_qt):
    ...
    add = sequence_transform([a_root], ast.Attribute(a_root, attr='Jets'))

    base_seq = ObjectStream(ast.Name(id='dude'))

    ref_var = astIteratorPlaceholder()
    s = add.sequence(base_seq, {a_root: ref_var}, mock_qt)
