...
def test_just_the_source(mock_root_sequence_transform):
    ...
    g.add_vertex(info=v_info(1, root_seq, Any, {a: a}))
...
def test_source_and_single_generator(mocker, mock_root_sequence_transform):
    ...
    g = Graph(directed=True)
    level_0 = g.add_vertex(info=v_info(1, root_seq, Any, {a1: astIteratorPlaceholder(0)}))
    ...
    seq_met.sequence.assert_called_with(mine, MatchASTDict({a1: astIteratorPlaceholder(0, [None])}))
...
def test_level_appropriate(mocker, mock_root_sequence_transform):
    ...
    mine, a1, root_seq = mock_root_sequence_transform
    g = Graph(directed=True)
    level_0 = g.add_vertex(info=v_info(0, root_seq, Any, {a1: astIteratorPlaceholder(0, [1])}))
